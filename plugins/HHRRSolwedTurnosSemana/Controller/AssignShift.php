...
class AssignShift extends Controller
{
    ...
    {
        $this->toolBox()->log()->info('HHRRSolwedTurnosSemana: Executing assign-shift action');
        ...
        try {
            ...
            if ($employeeShift->save()) {
                ...
                $this->toolBox()->log()->info('HHRRSolwedTurnosSemana: Shift ' . $shiftId . ' assigned to employee ' . $employeeId);
            } else {
                $this->toolBox()->i18nLog()->error('error-assigning-shift');
            }

        } catch (\Exception $e) {
            $this->toolBox()->log()->error('HHRRSolwedTurnosSemana: Error in assign-shift action: ' . $e->getMessage());
            $this->toolBox()->i18nLog()->error('error-assigning-shift');
        }
    ...
    {
        $this->toolBox()->log()->info('HHRRSolwedTurnosSemana: Executing unassign-shift action');
        ...
        try {
            ...
            if ($employeeShift->loadFromCode($assignmentId)) {
                if ($employeeShift->delete()) {
                    ...
                    $this->toolBox()->log()->info('HHRRSolwedTurnosSemana: Assignment ' . $assignmentId . ' deleted successfully');
                } else {
            ...
            $this->toolBox()->log()->error('HHRRSolwedTurnosSemana: Error in unassign-shift action: ' . $e->getMessage());
            $this->toolBox()->i18nLog()->error('error-unassigning-shift');
        }
    ...
    {
        $this->toolBox()->log()->info('HHRRSolwedTurnosSemana: Executing load-shifts-data action');
        ...
        try {
            ...
            $this->toolBox()->log()->error('HHRRSolwedTurnosSemana: Error in load-shifts-data action: ' . $e->getMessage());
