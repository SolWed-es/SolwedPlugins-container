...
class AssignShift extends Controller
{
    ...
    {
        $this->toolBox()->log()->info('HHRRSolwedTurnos: Executing assign-shift action');
        ...
        try {
            ...
            if ($employeeShift->save()) {
                ...
                $this->toolBox()->log()->info('HHRRSolwedTurnos: Shift ' . $shiftId . ' assigned to employee ' . $employeeId);
            } else {
                $this->toolBox()->i18nLog()->error('error-assigning-shift');
            }

        } catch (\Exception $e) {
            $this->toolBox()->log()->error('HHRRSolwedTurnos: Error in assign-shift action: ' . $e->getMessage());
            $this->toolBox()->i18nLog()->error('error-assigning-shift');
        }
    ...
    {
        $this->toolBox()->log()->info('HHRRSolwedTurnos: Executing unassign-shift action');
        ...
        try {
            ...
            if ($employeeShift->loadFromCode($assignmentId)) {
                if ($employeeShift->delete()) {
                    ...
                    $this->toolBox()->log()->info('HHRRSolwedTurnos: Assignment ' . $assignmentId . ' deleted successfully');
                } else {
            ...
            $this->toolBox()->log()->error('HHRRSolwedTurnos: Error in unassign-shift action: ' . $e->getMessage());
            $this->toolBox()->i18nLog()->error('error-unassigning-shift');
        }
    ...
    {
        $this->toolBox()->log()->info('HHRRSolwedTurnos: Executing load-shifts-data action');
        ...
        try {
            ...
            $this->toolBox()->log()->error('HHRRSolwedTurnos: Error in load-shifts-data action: ' . $e->getMessage());
