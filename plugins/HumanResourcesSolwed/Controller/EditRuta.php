...
use FacturaScripts\Plugins\HumanResourcesSolwed\Model\EmployeeRoute;
use FacturaScripts\Plugins\HumanResourcesSolwed\Model\Ruta;
...
class EditRuta extends EditController
{
            ...
            case self::VIEW_RUTA_DESTINOS:
                ...
                if (!empty($idruta)) {
                    ...
                    if (empty($view->model->orden) || $view->model->orden == 1) {
                        $view->model->orden = \FacturaScripts\Plugins\HumanResourcesSolwed\Model\RutaDestino::getNextOrder($idruta);
                    }
                }
                break;
    ...
    private function loadRutasDataAction()
    {
        ...
        try {
            ...
            $this->toolBox()->log()->error('HumanResourcesSolwed: Error in load-rutas-data action: ' . $e->getMessage());
            header('Content-Type: application/json');
            echo json_encode(['error' => $e->getMessage()]);
            exit;
        }
