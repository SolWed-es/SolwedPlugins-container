...
final class DonDominioContactService
{
    /**
     * Attempts to locate the DonDominio contact identifier associated with the given tax number.
     */
    ...
    public static function findContactsByTaxNumber(?string $taxNumber): array
    {
        ...
        try {
            ...
            if (!empty($data)) {
                $debugPath = Tools::folder('MyFiles', 'Tmp', 'dondominio_last_contact.json');
            ...
            Tools::log()->warning('dondominio-contact-lookup-error', ['%message%' => $exception->getMessage()]);
        }
