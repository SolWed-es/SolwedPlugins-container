...
/**
 * Extension for PresupuestoCliente model to add vehicle/machine support
 *
...
class PresupuestoCliente
{
    ...
    /**
     * Get the vehicle/machine associated with this estimation
     *
    * @return Vehiculo|null
    ...
    {
        return function() {
            // Validar que se haya seleccionado un vehículo (OBLIGATORIO)
            if (empty($this->idmaquina) && !empty($this->codcliente)) {
                $this->toolBox()->i18nLog()->warning('vehicle-required');
                return false;
            }

            // Validar que el coche pertenezca al cliente seleccionado
            if (!empty($this->idmaquina) && !empty($this->codcliente)) {
                $maquina = new Vehiculo();
                if ($maquina->loadFromCode($this->idmaquina)) {
